...
import com.sksamuel.avro4s.{AvroSchema, Decoder, Encoder, FromRecord, RecordFormat, SchemaFor, ToRecord}
...
private[serialization] object AvroRecordFormat {
  def apply[T: Encoder: Decoder: SchemaFor]: RecordFormat[T] = {
    val schema = AvroSchema[T]
    RecordFormat[T](schema)(using ToRecord[T](schema), FromRecord[T](schema))
  }
}
